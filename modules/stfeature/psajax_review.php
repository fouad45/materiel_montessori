...
if(Tools::getValue('action') == 'add-new-review')
{
	...
	if (!$context->customer->id && !Configuration::get('STFEATURE_PRODUCT_REVIEWS_ALLOW_GUESTS'))
	...
	if (!count($errors))
	{
		$customer_review = StProductReview::getByCustomer($id_product_review, $id_customer, true, $id_guest);
		if (!$customer_review || ($customer_review && (strtotime($customer_review['date_add']) + (int)Configuration::get('STFEATURE_PRODUCT_REVIEWS_MINIMAL_TIME')) < time()))
		{

			$review = new StProductReview();
			...
			{
				$grade_sum += $grade;
				$product_review_criterion = new StProductReviewCriterion($id_product_review_criterion);
			...
			$result = true;
			Tools::clearCache($context->smarty, $module->getTemplatePath('st_list_product_review.tpl'));
		}
		else
		{
			$result = false;
			$errors[] = $module->l('Please wait before posting another comment', 'psajax_review').' '.Configuration::get('STFEATURE_PRODUCT_REVIEWS_MINIMAL_TIME').' '.$module->l('seconds before posting a new review', 'psajax_review');
		}
	}
	else
		$result = false;
...
{	
	...
	if (StProductReview::isAlreadyUsefulness($id_product_review, Context::getContext()->cookie->id_customer))
		die('0');

	if (StProductReview::setReviewUsefulness((int)$id_product_review, (bool)$is_usefull, Context::getContext()->cookie->id_customer))
		die('1');
...
if(Tools::getValue('action') == 'add-review-report')
{
	...
	if (StProductReview::isAlreadyReport($id_product_review, Context::getContext()->cookie->id_customer))
		die('0');

	if (StProductReview::reportReview((int)$id_product_review, Context::getContext()->cookie->id_customer))
		die('1');
