...
class StWishList extends ObjectModel
{
	...
	{	
		Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'stfeature_wishlist_product` WHERE `id_wishlist` = '.(int)($this->id));
		if ($this->default)
		{
			$result = Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'stfeature_wishlist` WHERE `id_customer` = '.(int)$this->id_customer.' AND `id_wishlist` != '.(int)$this->id.' LIMIT 1');
	...
	public static function incCounter($id_wishlist)
	{
		...
		$result = Db::getInstance()->getRow('
			SELECT `counter`
			FROM `'._DB_PREFIX_.'stfeature_wishlist`
		...
		return Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'stfeature_wishlist` SET
	...
	* Get Customers having a wishlist
		...
		if (!Cache::isStored($cache_id))
		{
			$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				SELECT c.`id_customer`, c.`firstname`, c.`lastname`
				  FROM `'._DB_PREFIX_.'stfeature_wishlist` w
	...
	// {
		...
		// JOIN `'._DB_PREFIX_.'stfeature_wishlist_product` wp ON (wp.id_wishlist_product = wpc.id_wishlist_product)
		...
		// $freshwish = Db::getInstance()->executeS('
			...
			// FROM `'._DB_PREFIX_.'wishlist_product_cart` wpc
			// JOIN `'._DB_PREFIX_.'stfeature_wishlist_product` wp ON (wpc.id_wishlist_product = wp.id_wishlist_product)
			// JOIN `'._DB_PREFIX_.'cart` c ON (c.id_cart = wpc.id_cart)
		...
		// if (isset($res) AND $res != false)
			// foreach ($res AS $refresh)
				...
				// {
					// Db::getInstance()->execute('
						// UPDATE `'._DB_PREFIX_.'stfeature_wishlist_product`
		...
		// if (isset($freshwish) AND $freshwish != false)
			...
			// {
				// Db::getInstance()->execute('
					// UPDATE `'._DB_PREFIX_.'stfeature_wishlist_product` SET `quantity`=`quantity` +
					// (
	...
	public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
	{
		...
		FROM `'._DB_PREFIX_.'stfeature_wishlist_product` wp
		...
		LEFT JOIN `'._DB_PREFIX_.'stfeature_wishlist` w ON w.`id_wishlist` = wp.`id_wishlist`
	...
	public static function getInfosByIdCustomer($id_customer, $id_wishlist)
	{
		...
		SELECT SUM(wp.`quantity`) AS nbProducts, wp.`id_wishlist`
		  FROM `'._DB_PREFIX_.'stfeature_wishlist_product` wp
		INNER JOIN `'._DB_PREFIX_.'stfeature_wishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
	...
	public static function addProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute, $quantity)
	{
		...
		SELECT wp.`quantity`
		  FROM `'._DB_PREFIX_.'stfeature_wishlist_product` wp
		JOIN `'._DB_PREFIX_.'stfeature_wishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
		WHERE wp.`id_wishlist` = '.(int)($id_wishlist).'
		...
		if (empty($result) === false AND sizeof($result))
		{
			if (($result['quantity'] + $quantity) <= 0)
				...
				return (Db::getInstance()->execute('
				UPDATE `'._DB_PREFIX_.'stfeature_wishlist_product` SET
			...
			return (Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'stfeature_wishlist_product` (`id_wishlist`, `id_product`, `id_product_attribute`, `quantity`, `priority`) VALUES(
			'.(int)($id_wishlist).',
			'.(int)($id_product).',
	...
	public static function removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute)
	{
		...
		FROM `'._DB_PREFIX_.'stfeature_wishlist` w
		LEFT JOIN `'._DB_PREFIX_.'stfeature_wishlist_product` wp ON (wp.`id_wishlist` = w.`id_wishlist`)
		WHERE `id_customer` = '.(int)($id_customer).'
		...
		// );
		return Db::getInstance()->execute('
		DELETE FROM `'._DB_PREFIX_.'stfeature_wishlist_product`
		WHERE `id_wishlist` = '.(int)($id_wishlist).'
		AND `id_product` = '.(int)($id_product).'
		AND `id_product_attribute` = '.(int)($id_product_attribute)
		);
	...
	*/
	...
	{
		return (Bool)Db::getInstance()->getValue('SELECT * FROM `'._DB_PREFIX_.'stfeature_wishlist` WHERE `id_customer` = '.(int)$id_customer.' AND `default` = 1');
	}

	public static function getDefault($id_customer)
	{
		return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'stfeature_wishlist` WHERE `id_customer` = '.(int)$id_customer.' AND `default` = 1');
	}
	...
	*/
	public function setDefault()
	{
		if ($default = $this->getDefault($this->id_customer))
			Db::getInstance()->update('stfeature_wishlist', array('default' => '0'), 'id_wishlist = '.(int)$default[0]['id_wishlist']);

		return Db::getInstance()->update('stfeature_wishlist', array('default' => '1'), 'id_wishlist = '.(int)$this->id);
	}
