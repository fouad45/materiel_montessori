...
class StProductReviewCriterion extends ObjectModel
{
	public	$id;
	...
	const module_name = 'stfeature';

	/**
	 * @see ObjectModel::$definition
	 */
	public static $definition = array(
		'table' => 'stfeature_product_review_criterion',
	...
	public function delete()
	{
		...
		if ($this->id_product_review_criterion_type == 2)
		{
			if (!Db::getInstance()->execute('
					DELETE FROM '._DB_PREFIX_.'stfeature_product_review_criterion_category
		...
		{
			if (!Db::getInstance()->execute('
					DELETE FROM '._DB_PREFIX_.'stfeature_product_review_criterion_product
					WHERE id_product_review_criterion='.(int)$this->id))
				return false;
		}
	...
	public function addProduct($id_product)
	{
		...
		return Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'stfeature_product_review_criterion_product` (`id_product_review_criterion`, `id_product`)
	...
	public function addCategory($id_category)
	{
		if (!Validate::isUnsignedId($id_category))
			die(Tools::displayError());
		return Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'stfeature_product_review_criterion_category` (`id_product_review_criterion`, `id_category`)
	...
	public function addGrade($id_product_review, $grade)
	{
		...
		elseif ($grade > 10)
			$grade = 10;
		return (Db::getInstance()->execute('
		INSERT INTO `'._DB_PREFIX_.'stfeature_product_review_grade`
	...
	public static function getByProduct($id_product, $id_lang)
	{
		...
		$cache_id = 'StProductReviewCriterion::getByProduct_'.(int)$id_product.'-'.(int)$id_lang;
		if (!Cache::isStored($cache_id))
		{
			$result = Db::getInstance()->executeS('
				SELECT pcc.`id_product_review_criterion`, pccl.`name`
				FROM `'._DB_PREFIX_.'stfeature_product_review_criterion` pcc
				LEFT JOIN `'._DB_PREFIX_.'stfeature_product_review_criterion_lang` pccl
					ON (pcc.id_product_review_criterion = pccl.id_product_review_criterion)
				LEFT JOIN `'._DB_PREFIX_.'stfeature_product_review_criterion_product` pccp
					ON (pcc.`id_product_review_criterion` = pccp.`id_product_review_criterion` AND pccp.`id_product` = '.(int)$id_product.')
				LEFT JOIN `'._DB_PREFIX_.'stfeature_product_review_criterion_category` pccc
	...
	public static function getCriterions($id_lang, $type = false, $active = false)
	{
		...
		$sql = '
			...
			FROM `'._DB_PREFIX_.'stfeature_product_review_criterion` pcc
			JOIN `'._DB_PREFIX_.'stfeature_product_review_criterion_lang` pccl ON (pcc.id_product_review_criterion = pccl.id_product_review_criterion)
	...
	public function getProducts()
	{
		$res = Db::getInstance()->executeS('
			...
			FROM `'._DB_PREFIX_.'stfeature_product_review_criterion_product` pccp
	...
	public function getCategories()
	{
		$res = Db::getInstance()->executeS('
			...
			FROM `'._DB_PREFIX_.'stfeature_product_review_criterion_category` pccc
