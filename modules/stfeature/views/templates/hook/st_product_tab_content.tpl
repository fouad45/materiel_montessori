		...
		<div id="product_reviews_block_tab">
					...
					<div class="review" itemprop="review" itemscope itemtype="https://schema.org/Review">
						<div class="review-info row">
							<div class="review_author col-sm-3">
								<span>{l s='Grade' mod='stfeature'}&nbsp;</span>
						...
						<div class="review_button">
							<ul>
								{if $review.total_advice > 0}
									<li>
										{l s='%1$d out of %2$d people found this review useful.' sprintf=[$review.total_useful,$review.total_advice] mod='stfeature'}
									</li>
								{/if}
								{if $customer.is_logged}
									{if !$review.customer_advice && $allow_usefull_button}
									<li>
										{l s='Was this review useful to you?' mod='stfeature'}
										<button class="usefulness_btn btn btn-default button button-small" data-is-usefull="1" data-id-product-review="{$review.id_product_review}">
											<span>{l s='Yes' mod='stfeature'}</span>
										</button>
										<button class="usefulness_btn btn btn-default button button-small" data-is-usefull="0" data-id-product-review="{$review.id_product_review}">
											<span>{l s='No' mod='stfeature'}</span>
										</button>
									</li>
									{/if}
									...
									<li>
										<a href="#" class="btn report_btn" data-id-product-review="{$review.id_product_review}">
											{l s='Report abuse' mod='stfeature'}
										</a>
									</li>
									{/if}
								{/if}
			...
			{else}
				{if (!$too_early AND ($customer.is_logged OR $allow_guests))}
					<a class="open-review-form" href="#" data-id-product="{$id_product_tab_content}" data-is-logged="{$customer.is_logged}" data-product-link="{$link_product_tab_content}">
						<i class="material-icons">&#xE150;</i>
						{l s='Be the first to write your review!' mod='stfeature'}
					</a>			
				{else}
					<p class="align_center">{l s='No customer reviews for the moment.' mod='stfeature'}</p>
				{/if}
			{/if}
		</div> 
{if isset($USE_PTABS) && $USE_PTABS == 'default'}
