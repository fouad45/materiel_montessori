<div class="checkoutStep2">
    <div class="">
        <form class="checkout-form" style="{if !$is_logged}display:none;{/if}">
            ...
            <div class="container-fluid">
                <div class="saved-address mb-2 clearfix">
                    <div class="row">
                        <div class="col-10">
                ...
                <div class="saved-address mb-2 clearfix" id="delivery-address-list">
                    {foreach from=$addresses key=k item=address}
                    <div class="row mb-2" id="delivery-address-{$address.id_address}">
                        <div class="col-10">
                            ...
                            {/if}
                            <div>
                                <a class="blue address-edit" onclick="shipping.edit({$address|@json_encode|escape:'html':'UTF-8'}, false, false, {!$is_logged})">{l s='Edit address details' mod='jmango360api'}</a>
                            </div>
                        </div>
                        <div class="col-2 text-center">
                            <label class="jm-radio-check mb-0">
                                <input type="radio" name="selected-shipping-address" value="{$address.id_address}" id="selected-shipping-address-{$address.id_address}" {if $address.id_address|intval === $selected_address|intval}checked{/if}>
                                <span class="radio-check"></span>
                            </label>
                        </div>
                    </div>
            ...
            <div class="new-address">
                <a class="blue address-edit" onclick="shipping.edit(null, false, {$default_country_id}, {!$is_logged})">{l s='Use different shipping address' mod='jmango360api'}</a>
            </div>
    ...
    <div id="shipping-address-details" class="shipping-address-details {if $is_logged}overlay-window{/if}" style="{if $is_logged}display:none{else}position: inherit {/if}">
        ...
        {include file="$template_dir/onepage17_new/steps/shipping-address.tpl"}
    </div>
</div>
    ...
    <div class="confirmDialogInner" >
        <div class="confirmDialogContent">
            ...
            <div class="confirmDialogBtn">
                ...
                <button type="button" class="btn btn-primary" onclick="$('#parent-div').removeClass('showAddressForm');$('body').removeClass('showAddressForm');$('#shippingConfirmDialog').hide(); shipping.closeAddressFrom()">{l s='Yes' mod='jmango360api'}</button>
            </div>
        </div>
    </div>
</div>
