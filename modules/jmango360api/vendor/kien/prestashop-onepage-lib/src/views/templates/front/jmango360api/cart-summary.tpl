...
<div id="checkout-review-table-wrapper">
        <table class="data-table" id="checkout-review-table">
                ...
                <tr class="jm-item">
                    <td class="jm-item-img">
                        <div class="jm-item-img-inner">
                            <span class="jm-item-qty">{$product.cart_quantity|escape:'html':'UTF-8'}</span>
                    ...
                    <span class="jm-item-desc-item">
                        {if $product.reference}<small>{l s='SKU' mod='jmango360api'} : {$product.reference|escape:'html':'UTF-8'}</small>{/if}
                    </span>
                    <span class="jm-item-desc-item">
            ...
            <tr>
                {if $use_taxes}
                    {if $priceDisplay}
                        <td style="" class="a-right" colspan="2">{if $display_tax_label}{l s='Total products (tax excl.)' mod='jmango360api'}{else}{l s='Total products' mod='jmango360api'}{/if}</td>
                        <td style="" class="a-right">
                            <span class="price">{displayPrice price=$total_products}</span>
                        </td>
                    {else}
                        <td style="" class="a-right" colspan="2">{if $display_tax_label}{l s='Total products (tax incl.)' mod='jmango360api'}{else}{l s='Total products' mod='jmango360api'}{/if}</td>
                        <td style="" class="a-right"><span class="price">{displayPrice price=$total_products_wt}</span></td>
                    {/if}
                {else}
                    <td style="" class="a-right" colspan="2">{l s='Total products' mod='jmango360api'}</td>
                    <td style="" class="a-right">
                        <span class="price">{displayPrice price=$total_products}</span>
                    </td>
                {/if}
            ...
            <tr>
                {if $total_shipping_tax_exc <= 0 && (!isset($isVirtualCart) || !$isVirtualCart) && $free_ship}
                    <td style="" class="a-right" colspan="2">
                        {l s='Total shipping' mod='jmango360api'}
                    </td>
                    <td style="" class="a-right">
                        <span class="price">{l s='Free Shipping' mod='jmango360api'}</span>
                    </td>
                {else}
                    {if $use_taxes && $total_shipping_tax_exc != $total_shipping}
                        {if $priceDisplay}
                            <td style="" class="a-right" colspan="2">
                                {if $display_tax_label}{l s='Total shipping (tax excl.)' mod='jmango360api'}{else}{l s='Total shipping' mod='jmango360api'}{/if}
                            </td>
                            <td style="" class="a-right">
                        ...
                        {else}
                            <td style="" class="a-right" colspan="2">
                                {if $display_tax_label}{l s='Total shipping (tax incl.)' mod='jmango360api'}{else}{l s='Total shipping' mod='jmango360api'}{/if}
                            </td>
                            <td style="" class="a-right">
                ...
                <td style="{if $total_discounts == 0}display:none{/if}" class="a-right" colspan="2">
                    {if $display_tax_label}
                        {if $use_taxes && $priceDisplay == 0}
                            {l s='Total vouchers (tax incl.)' mod='jmango360api'}
                        {else}
                            {l s='Total vouchers (tax excl.)' mod='jmango360api'}
                        {/if}
                    {else}
                        {l s='Total vouchers' mod='jmango360api'}
                    {/if}
                </td>
            ...
            {if $use_taxes && $show_taxes && $total_tax != 0 }
                {if $priceDisplay != 0}
                    <tr>
                        <td style="" class="a-right" colspan="2">
                            <strong>{if $display_tax_label}{l s='Total (tax excl.)' mod='jmango360api'}{else}{l s='Total' mod='jmango360api'}{/if}</strong>
                        </td>
                        <td style="" class="a-right">
                ...
                <tr>
                    <td style="" class="a-right" colspan="2">
                        <strong>{l s='Tax' mod='jmango360api'}</strong>
                    </td>
                    <td style="" class="a-right">
            ...
            <tr>
                <td style="" class="a-right" colspan="2">
                    <strong>{l s='Total' mod='jmango360api'}</strong>
            ...
            <form action="" id="checkout-agreements" class="checkout-agreements" onsubmit="return false;">
                <dl class="sp-methods">
                    <dt>
                        ...
                        <label for="agreement-1">
                            {l s='I agree to the terms of service and will adhere to them unconditionally.' mod='jmango360api'}
                            <a href="#" data-toggle="modal" data-target="#agreementModal1" class="agreement-label"> {l s='(Read the Terms of Service)' mod='jmango360api'}</a>
                        </label>
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h1 class="modal-title" id="agreementModalLabel1">
                                        {l s='Terms of Service' mod='jmango360api'}
                                    </h1>
                                </div>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{l s='Close' mod='jmango360api'}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <script>
                    ...
                    $modal.on('show.bs.modal', function(e) {
                        $(this)
                            .find('.modal-body')
                            .html('<p class="saving">{l s='Loading' mod='jmango360api' js=1}<span>.</span><span>.</span><span>.</span></p>')
                        ...
                        <li>
                            ...
                            <div class="buttons-set">
                                <button type="button" class="ladda-button" data-color="jmango" data-size="xs" data-style="slide-up" onclick="coupon.save(false)" style="width:auto;" id="coupon-button" title="Apply Coupon" value="Apply Coupon">
                                    <span class="ladda-label">{l s='Apply' mod='jmango360api'}</span></button>
                            </div>
                        </li>
                    </ul>
                </dd>
