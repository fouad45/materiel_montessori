<div class="checkoutStep1" id="checkoutStep1">
    <div class="">
        <form id="billing-address-form-list" class="checkout-form" style="{if !$is_logged || $addresses|@count === 0}display:none;{/if}">
            ...
            <div class="saved-address-list" id="invoice-address-list">
                {foreach from=$addresses key=k item=address}
                    <div class="saved-address mb-2 clearfix" id="invoice-address-{$address.id_address}">
                        <div class="row positionRelative">
                            ...
                            <div class="col-2 col-xs-2 positionStatic text-center">
                                <label class="jm-radio-check mb-0" id="billing-radio-{$address.id_address}">
                                </label>
                            </div>
                        </div>
                    </div>
    ...
    <div id="billing-address-details" class="billing-address-details {if $is_logged && $addresses|@count != 0}overlay-window{/if}" style="{if $is_logged && $addresses|@count !== 0}display:none{else}position: inherit {/if}">
        <header id="billing-address-details-header" class="overlay-header" style="{if !$is_logged || $addresses|@count == 0}display:none;{/if}">
            ...
            <h1>{l s='Billing Address' mod='jmango360api'}</h1>
        </header>
        {include file="$template_dir/onepage16_new/steps/billing-address.tpl"}
    </div>
</div>
    ...
    <div class="confirmDialogInner" >
        <div class="confirmDialogContent">
            ...
            <div class="confirmDialogBtn">
                ...
                <button type="button" class="btn btn-primary" onclick="$('#billingConfirmDialog').hide(); billing.closeAddressFrom()">{l s='Yes' mod='jmango360api'}</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    {if !$is_logged || $addresses|@count === 0}
        billing.edit(null, false, {$default_country_id}, true);
        $("#parent-div").removeClass("showAddressForm");$("body").removeClass("showAddressForm");
    {/if}
    $(document).ready(function () {
        billing.renderCheckbox(addresses, {$selected_invoice_address_id});
    });
</script>