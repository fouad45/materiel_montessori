              ...
              <span></span>
                    ...
                    >
                        ...
                        {if $option.form}
                            {$option.form}
                        {else}
                            <form id="payment-form-submit-{$option.id|escape:'html':'UTF-8'}" method="POST"
                                  action="{$option.action|escape:'html':'UTF-8'}">
                                {foreach from=$option.inputs item=input}
                                    <input type="{$input.type|escape:'html':'UTF-8'}" name="{$input.name|escape:'html':'UTF-8'}"
                                           value="{$input.value|escape:'html':'UTF-8'}">
                                {/foreach}
                                <button style="display:none" id="pay-with-{$option.id|escape:'html':'UTF-8'}"
                                        type="submit"></button>
                            </form>
                        {/if}
                    </div>
