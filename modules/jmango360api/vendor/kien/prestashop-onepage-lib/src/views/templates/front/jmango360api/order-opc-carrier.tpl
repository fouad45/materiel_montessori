...
<script type="text/javascript">
    ...
    var txtProduct = "{l s='product' mod='jmango360api' js=1}";
    var txtProducts = "{l s='products' mod='jmango360api' js=1}";
    var orderUrl = '{$link->getPageLink("order", true)|escape:'html'}';
    var msg = "{l s='You must agree to the terms of service before continuing.' mod='jmango360api' js=1}";
...
<div data-role="content" id="delivery_choose">
    <h3 class="bg">{l s='Choose your delivery method' mod='jmango360api'}</h3>
                                    ...
                                    {/if}
										...
										<input type="hidden" value="{$carrier.instance->id|intval}" name="id_carrier"/>
                                            ...
                                            {if count($carrier.product_list) <= 1}
                                                ({l s='Product concerned:' mod='jmango360api'}
                                            {else}
                                                ({l s='Products concerned:' mod='jmango360api'}
                                            {/if}
    ...
    <fieldset data-role="fieldcontain">
        ...
        <label for="recyclable">{l s='I agree to receive my order in recycled packaging' mod='jmango360api'}.</label>
    </fieldset>

    {if $giftAllowed}
        <h3 class="gift_title">{l s='Gift' mod='jmango360api'}</h3>
        <p class="checkbox">
            <input type="checkbox" name="gift" id="gift" value="1" {if $cart->gift == 1}checked="checked"{/if}
                   autocomplete="off"/>
            <label for="gift">{l s='I would like my order to be gift wrapped.' mod='jmango360api'}</label>
            <br/>
            {if $gift_wrapping_price > 0}
                ({l s='Additional cost of' mod='jmango360api'}
                <span class="price" id="gift-price">
					{if $priceDisplay == 1}{convertPrice price=$total_wrapping_tax_exc_cost}{else}{convertPrice price=$total_wrapping_cost}{/if}
				</span>
                {if $use_taxes}{if $priceDisplay == 1} {l s='(tax excl.)' mod='jmango360api'}{else} {l s='(tax incl.)' mod='jmango360api'}{/if}{/if})
            {/if}
        </p>
    ...
    <h3 class="bg">{l s='Terms of service' mod='jmango360api'}</h3>
    <fieldset data-role="fieldcontain" id="cgv_checkbox">
        ...
        <label for="cgv">{l s='I agree to the terms of service and will adhere to them unconditionally.' mod='jmango360api'}</label>
    </fieldset>
    <p class="lnk_CGV"><a href="{$link_conditions|escape:'url'}" data-ajax="false">{l s='(Read Terms of Service)' mod='jmango360api'}</a></p>
</div>
