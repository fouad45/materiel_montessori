...
{block name='step_content'}
    {if $customer.is_logged && !$customer.is_guest}
        ...
        {if !isset($empty_cart_on_logout) || $empty_cart_on_logout}
            <p><small>{l s='If you sign out now, your cart will be emptied.' mod='jmango360api'}</small></p>
        {/if}
    ...
    {else}
        ...
        <ul class="nav nav-inline my-2" role="tablist">
            <li class="nav-item">
                ...
                >
                    {if $guest_allowed}
                        {l s='Order as a guest' mod='jmango360api'}
                    {else}
                        {l s='Create an account' mod='jmango360api'}
                    {/if}
                </a>
            </li>
