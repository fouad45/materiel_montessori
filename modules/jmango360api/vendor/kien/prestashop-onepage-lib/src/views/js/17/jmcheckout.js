...
var ShippingMethod = function (form, url) {
    ...
    self.constructor = function (form, url) {
        self.url = url;
        self.form = $('#' + form);
        $(document).delegate('button[name="confirmDeliveryOption"]', 'click', self.save);
    };
    ...
    self.save = function () {
        ...
        if (self.form.isValid()) {
            ...
            var formData = $('#js-delivery').serialize();
            $.ajax({
                type: 'POST',
                headers: {
                    "cache-control": "no-cache"
...
var Review = function (paymentForm, agreementsForm, url) {
    ...
    self.submit = function () {
        ...
        var payment_module_real_name = $('input:radio[name="payment-option"]:checked').data('module-name');

        var $form = $('#payment-form-submit-' + payment_module_name);
        if ($form.length) $form.submit();

        if (payment_module_real_name === 'sequrainvoice') {
            // Donot disable submit button
        } else {
