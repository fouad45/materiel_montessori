...
class JmCheckoutShippingAddressStep extends JmCheckoutStep
{
    ...
    public function __construct(
        ...
        $this->template_dir = _PS_MODULE_DIR_ .$checkoutController->module_name. "/vendor/kien/prestashop-onepage-lib/src/views/templates/front/".$checkoutController->module_name;
        $this->template = 'module:'.$checkoutController->module_name.'/vendor/kien/prestashop-onepage-lib/src/views/templates/front/'.$checkoutController->module_name.'/onepage17_new/steps/address-form.tpl';
    ...
    public function handleRequest(array $requestParameters = array())
    {
        ...
        $saved = true;
        if (!isset($requestParameters['isSubmit']) || !(int)$requestParameters['isSubmit']){
        ...
        if (!$saved) {
            ...
            if (isset($requestParameters['isSubmit']) && (int)$requestParameters['isSubmit']) {
                $id_address = $requestParameters['shipping_address_id'];
            } else {
                $id_address = $this->addressForm->getAddress()->id;
            }
            if (isset($requestParameters['saveAddress']) && $requestParameters['saveAddress'] === 'delivery') {
