...
class FinalJmCheckout17_old extends ModuleFrontController
{
    ...
    public function init()
    {
        ...
        $this->template = 'module:' . $this->module_name . '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage17/jmcheckout.tpl';
        ...
        if (version_compare(_PS_VERSION_, '1.7.1', '<') && count($jsAllowedList)) {
            foreach ($jsAllowedList as $jsFileId => $jsFilePath) {
                $this->context->controller->registerJavascript($jsFileId, $jsFilePath, array(
                    'position' => 'head', 'attributes' => 'defer'
                ));
            }
        }
    ...
    public function setMedia()
    {
        ...
        $this->registerJavascript(
            ...
            array('position' => 'head', 'priority' => 150, 'attributes' => 'defer')
        );
        $this->registerJavascript(
        ...
        $this->registerJavascript(
            ...
            array('position' => 'head', 'priority' => 999, 'attributes' => 'defer')
        );
        $this->registerJavascript(
        ...
        if (strpos($_SERVER['HTTP_HOST'], 'misstic.fr') !== false) {
            if (Module::isEnabled('soflexibilite')) {
                ...
                if (Configuration::get('SOFLEXIBILITE_GMAP')) {
                    $this->context->smarty->assign('custom_html',
                        sprintf('<script src="%s"></script>', 'https://maps.google.com/maps/api/js?libraries=places&key=' . Configuration::get('SOFLEXIBILITE_GMAP_KEY'))
                    );
                }
            }
            ...
            if (Module::isEnabled('mondialrelayadvanced')) {
                ...
                if (isset($module_conf['google_map']) && $module_conf['google_map']) {
                    $this->context->smarty->assign('custom_html',
                        sprintf('<script src="%s"></script>', 'https://maps.google.com/maps/api/js?libraries=places&key=' . $module_conf['google_map_key'])
                    );
                }
            }
        }
        ...
        if(Module::isEnabled('chronopost')){
            ...
            if(Configuration::get('CHRONOPOST_MAP_APIKEY')) {
                $this->registerJavascript(
                    'chronopost-google.js',
                    'https://maps.googleapis.com/maps/api/js?key='. Configuration::get('CHRONOPOST_MAP_APIKEY'),
                    array('position' => 'head', 'priority' => 80, 'server' => 'remote')
                );
            }
        }
    ...
    {
        $this->initFirstForm();
        $this->personalInformationStep = new JMCheckoutPersonalInformationStepOld(
            $this->context,
            $this->getTranslator(),
        ...
        $this->billingAddressStep = new JMCheckoutBillingAddressStepOld(
            $this->context,
            $this->getTranslator(),
            $this->jmCheckoutSession,
            $this->addressForm,
    ...
    {
        $this->initFirstForm();
        $this->shippingAddressStep = new JmCheckoutShippingAddressStepOld(
            $this->context,
            $this->getTranslator(),
            $this->jmCheckoutSession,
            $this->addressForm,
    ...
    {
        $shippingMethodStep = new JmCheckoutShippingMethodStepOld(
            $this->context,
            $this->getTranslator(),
            $this->jmCheckoutSession,
            $this
    ...
    private function getPayment($requestParameters)
    {
        ...
        $this->paymentStep = new JmCheckoutPaymentStepOld(
            $this->context,
            $translator,
            $this->jmCheckoutSession,
    ...
    private function loadSummary()
    {
        ...
        */
        ...
        $this->reviewStep = new JmCheckoutReviewStepOld(
            $this->context,
            $this->getTranslator(),
            $this->jmCheckoutSession,
            $conditionFinder,
