...
class JmCheckout16 extends ModuleFrontController
{
    ...
    public function init()
    {
        ...
        if (CheckoutSettingsService::getDisableMergeCss()) {
            Configuration::set('PS_CSS_THEME_CACHE', 0);
        }
        if (CheckoutSettingsService::getDisableMergeJs()) {
            Configuration::set('PS_JS_THEME_CACHE', 0);
        }

        /**
         * PS-1637: Support 'psgdpr' module
         */
        if (Module::isEnabled('psgdpr')) {
            $_POST['psgdpr-consent'] = 1;
        }

        if (strpos($_SERVER['HTTP_HOST'], 'dampwinkeltje.nl') !== false) {
...
//        $id_address_invoice = 0;
        ...
        $this->template = $this->template_dir . '/onepage16_new/jmcheckout.tpl';
        ...
        $this->context->smarty->assign('default_country_id', $this->getDefaultCountry());
        ...
        $output = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module_name . '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/billing-address-form.tpl');
        ...
        if (!count($this->errors)) {
            if ($current_step == 'billing') {
                if ($id_billing_address == 0
                    ...
                    // Check phone
                    if (Configuration::get('PS_ONE_PHONE_AT_LEAST') && empty($billing_params['phone']) && empty($billing_params['phone_mobile'])) {
                ...
                if ($id_shipping_address == 0
                    ...
                    // Check phone
                    if (Configuration::get('PS_ONE_PHONE_AT_LEAST') && empty($shipping_params['phone']) && empty($shipping_params['phone_mobile'])) {
                        $this->errors[] = Tools::displayError('You must register at least one phone number.');
                    }
                }
            }
...
//                            }
    ...
    public function setMedia()
    {
        ...
        $this->addCSS($this->module_dir . 'views/css/16/bootstrap.min.css');
        $this->addCSS($this->module_dir . 'views/css/font-awesome-4.7.0/css/font-awesome.css');
        $this->addCSS($this->module_dir . 'vendor/kien/prestashop-onepage-lib/src/views/css/style_new.css');
        ...
        $this->addJS($this->module_dir . 'views/js/libs/bootstrap.min.js');
        ...
        $this->addJS($this->module_dir . 'vendor/kien/prestashop-onepage-lib/src/views/js/16_new/jmcheckout.js?v=6');
    ...
    protected function loadPayments(
        ...
        if ($methods) {
            foreach ($methods as $module) {
                /**
                 * PS-489: Hide Unsupported Payment Method - Amazon payments
                 */
                ...
                if (ModuleCore::isEnabled('onepagecheckoutps')) {
                    $paymentClass = _PS_MODULE_DIR_ . 'onepagecheckoutps/classes/PaymentClass.php';
                    if (file_exists($paymentClass)) {
                        require_once $paymentClass;

                        $id_payment = PaymentClass::getIdPaymentBy('id_module', (int)$module['id_module']);
                        if (!empty($id_payment)) {
                            $payment_instance = new PaymentClass($id_payment, $context->language->id);
                        }
                    }
                }
                ...
                if (is_callable(array($module_instance, 'hookpayment')) || is_callable(array($module_instance, 'hookDisplayPayment'))) {
                    ...
                    if (!$module_instance->currencies ||
                        ($module_instance->currencies && count(Currency::checkPaymentCurrencies($module_instance->id)))
                    ) {
                        ...
                        if (!empty($html)) {
                            $paymentsResult = CommonUtils::parsePaymentFromHtml($html, $module['name']);
                            foreach ($paymentsResult as $paymentResult) {
                                $payment_module_url = isset($paymentResult['url']) ? $paymentResult['url'] : '';
                                $display_name = isset($paymentResult['description'])
                                    ? $paymentResult['description']
                                    : (isset($paymentResult['title']) ? $paymentResult['title'] : '');
                                $payment_image_url = isset($paymentResult['logo']) ? $paymentResult['logo'] : '';
                                ...
                                /**
                                 * PS-1310: Get alternative description
                                 */
                                if ($module['name'] == 'paypal' && $payment_instance) {
                                    $display_name = $payment_instance->description;
                                }
        ...
        if (!count($this->errors)) {
            ...
            $payment_html = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module_name .
                '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage16_new/steps/step4-payment-method.tpl');
        }
...
//                'additional' => $additional_payment_methods
    ...
    private function updateCarrier()
    {
        ...
        $paymentHtmlContent = '';
        $paymentContent = $this->loadPayments($shipping_address->id_country, $shipping_address->id, 0);
...
//        $this->context->smarty->assign('payment_methods', $this->loadPaymentMethods());
        $cart_count = Cart::getNbProducts($this->context->cart->id);
        $cart_count_label = "";
        if ($cart_count > 1) {
            $cart_count_label = $cart_count . ' ' . $this->module->l('items', 'step5-order-review');
        } else {
            $cart_count_label = $cart_count . ' ' . $this->module->l('item', 'step5-order-review');
        }
        $this->context->smarty->assign('cart_count_label', $cart_count_label);
        $cart_summary_html = '';
        if (empty($this->errors)) {
            $this->_assignSummaryInformations();
            $cart_summary_html = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module_name .
                '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage16_new/steps/step5-order-review.tpl');
        }
    ...
    private function getCarrierList(
        ...
        $this->default_shipping_selected = Configuration::get('PS_CARRIER_DEFAULT');
        ...
        if (Validate::isLoadedObject($delivery_address) && count($this->shipping_error) == 0) {
            ...
            if (!count($this->shipping_error)) {
                ...
                /**
                 * PS-1583: Support auto select shipping method if only one
                 */
                if (count($delivery_option) == 1) {
                    foreach ($delivery_option as $index => $item) {
                        $this->default_shipping_selected = trim($item, ' ,');
                    }
                }
...
//            return $arr;
        ...
        $no_carrier_message = $this->module->l(
            'There are no carriers that deliver to the address you selected.',
            'step3-shipping-method'
        );

        $this->context->smarty->assign("no_carrier_message", $no_carrier_message);
        ...
        $temp_vars = array(
            ...
            'carrier_block' =>
                $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module_name .
                    '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage16_new/steps/step3-shipping-method.tpl'),
...
//            'displayVouchers' => $available_cart_rules,
    ...
    private function placeOrder()
    {
        ...
        if (is_array($product)) {
            $this->errors[] = htmlspecialchars_decode(sprintf(Tools::displayError(
                'An item (%1s) in your cart is no longer available in this quantity. You cannot proceed with your order until the quantity is adjusted.'
...
//        }
    ...
    private function initFormFieldBillingAddress($id_country)
    {
        ...
        if ($this->isAjax()) {
            ...
            $this->context->smarty->assign('one_phone_at_least', (int)Configuration::get('PS_ONE_PHONE_AT_LEAST'));
            $this->context->smarty->assign('current_country', (int)$id_country);
            ...
            if (Tools::getValue('step') == 'billing') {
                $output = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module_name . '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage16_new/steps/form-fields.tpl');
            } elseif (Tools::getValue('step') == 'shipping') {
                $output = $this->context->smarty->fetch(_PS_MODULE_DIR_ . $this->module_name . '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage16_new/steps/form-fields-shipping.tpl');
            }
            $this->context->smarty->assign('output', ($output));
...
//            }
    ...
    private function getAddressFormFields($type, $id_country = null)
    {
        ...
        $html = '';
        try {
            $html = $smarty->fetch(_PS_MODULE_DIR_ . $this->module_name . '/vendor/kien/prestashop-onepage-lib/src/views/templates/front/' . $this->module_name . '/onepage16_new/steps/form-fields-shipping.tpl');
        } catch (Exception $e) {
            ob_end_clean();
    ...
    private function updateAddress($addressUpdate, $address_id = null)
    {
        ...
        $address->id_customer = (int)$this->context->customer->id;
        $address->id = null;
        $required_fields = $address->getFieldsRequiredDB();
        foreach ($required_fields as $required_field) {
            if (!$address_old->$required_field) {
                $address_old->$required_field = '0';
            }
        }
...
//                }
        ...
        if (!count($this->errors)) {
            if ($current_step == 'billing') {
                ...
                if ($id_billing_address == 0
                    ...
                    $billing_address->company = (!empty($billing_params['company'])) ? $billing_params['company'] : ' ';
                    $billing_address->vat_number = (!empty($billing_params['vat_number'])) ? $billing_params['vat_number'] : ' ';
                    $billing_address->address1 = (!empty($billing_params['address1'])) ? $billing_params['address1'] : ' ';
                    $billing_address->address2 = (!empty($billing_params['address2'])) ? $billing_params['address2'] : ' ';
                    $billing_address->city = (!empty($billing_params['city'])) ? $billing_params['city'] : ' ';
                    $billing_address->phone = (!empty($billing_params['phone'])) ? $billing_params['phone'] : '';
            ...
            if ($current_step == 'shipping') {
                ...
                if ($id_shipping_address == 0
                    ...
                    $shipping_address->vat_number = (!empty($shipping_params['vat_number'])) ? $shipping_params['vat_number'] : ' ';
                    $shipping_address->address1 = (!empty($shipping_params['address1'])) ? $shipping_params['address1'] : ' ';
                    $shipping_address->address2 = (!empty($shipping_params['address2'])) ? $shipping_params['address2'] : ' ';
                    $shipping_address->city = (!empty($shipping_params['city'])) ? $shipping_params['city'] : ' ';
                    $shipping_address->phone = (!empty($shipping_params['phone'])) ? $shipping_params['phone'] : ' ';
    ...
    public function createGuestAccount()
    {
        ...
        // PS-861 : [Ps 16] Onepage checkout - Display wrong error message  in case I input invalid Date of birth.
        if (isset($billing_params['years']) && isset($billing_params['months']) && isset($billing_params['days'])) {
            $birthDay = (empty($billing_params['years']) ? '' : (int)$billing_params['years'] . '-' . (int)$billing_params['months'] . '-' . (int)$billing_params['days']);
            if (!@checkdate($billing_params['months'], $billing_params['days'], $billing_params['years']) && !($billing_params['months'] == '' && $billing_params['days'] == '' && $billing_params['years'] == '')) {
                $this->errors[] = Tools::displayError('Invalid date of birth');
            } else {
    ...
    protected function processValidationGuestAcount($billing_params)
    {
        if ((!count($this->errors)) && (!$this->is_logged)) {
            $address = new Address();
            $address->id_country = (isset($billing_params['country_id']) && ($billing_params['country_id'] != '')) ? $billing_params['country_id'] : null;
            $address->address2 = (isset($billing_params['address2']) && ($billing_params['address2'] != ' ')) ? $billing_params['address2'] : null;
            $address->address1 = (isset($billing_params['address1']) && ($billing_params['address1'] != "")) ? $billing_params['address1'] : null;
            $address->firstname = (isset($billing_params['firstname']) && ($billing_params['firstname'] != '')) ? $billing_params['firstname'] : null;
            $address->lastname = (isset($billing_params['lastname']) && ($billing_params['lastname'] != '')) ? $billing_params['lastname'] : null;
            $address->city = (isset($billing_params['city']) && ($billing_params['city'] != '')) ? $billing_params['city'] : null;
            $address->company = (isset($billing_params['company']) && ($billing_params['company'] != '')) ? $billing_params['company'] : null;
            $address->postcode = (isset($billing_params['postcode']) && ($billing_params['postcode'] != '')) ? $billing_params['postcode'] : null;
            $address->other = (isset($billing_params['other']) && ($billing_params['other'] != '')) ? $billing_params['other'] : null;
            $address->phone = (isset($billing_params['phone']) && ($billing_params['phone'] != '')) ? $billing_params['phone'] : null;
            $address->phone_mobile = (isset($billing_params['phone_mobile']) && ($billing_params['phone_mobile'] != '')) ? $billing_params['phone_mobile'] : null;
            $address->dni = (isset($billing_params['dni']) && ($billing_params['dni'] != '')) ? $billing_params['dni'] : null;
            $address->vat_number = (isset($billing_params['vat_number']) && ($billing_params['vat_number'] != '')) ? $billing_params['vat_number'] : null;
            $this->errors = array_unique(array_merge($this->errors, $address->validateController()));
...
//                $id_country = $countries['0']['id_country'];
    ...
    public function assignAddresses()
    {
        ...
        $delivery_option = $this->context->cart->getDeliveryOption(null, false, false);
        $selected_delivery_option = $delivery_option[$this->context->cart->id_address_delivery];
        $delivery_option_list = $this->context->cart->getDeliveryOptionList();
        $this->context->smarty->assign('billing_address', $address_invoice);
        $this->context->smarty->assign('shipping_address', $address_delivery);
        $this->context->smarty->assign('delivery_option', $delivery_option_list[$this->context->cart->id_address_delivery][$selected_delivery_option]);
        $this->context->smarty->assign('module_template_dir', _PS_MODULE_DIR_ . $this->module_name . "/vendor/kien/prestashop-onepage-lib/src/views/templates/front/" . $this->module_name);
    }
}
