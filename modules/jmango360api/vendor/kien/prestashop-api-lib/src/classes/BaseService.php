...
abstract class BaseService implements JapiService
{
    ...
    /**
     * PS-1420: Init country from cart billing address
     */
    ...
    protected function prepareLanguage()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7', '>=')) {
            ...
            if (version_compare(_PS_VERSION_, '1.7.6', '>=')) {
                ...
                $this->context->currentLocale = $localeRepository->getLocale($language['language_code']);
            }
        }
    }
...
//        return Translate::getModuleTranslation('jmango360api', $string, $source);
    ...
    /**
     * FLag cart as mobile for later use
     *
     * @param $id_cart
     * @throws
     */
    public function markCartFromMobile($id_cart)
    {
        if (!$id_cart) return;

        if ($this->checkCartFromMobile($id_cart)) return;
    ...
    /**
     * Check if cart generated from mobile
     *
     * @param $id_cart
     * @return bool
     */
    public function checkCartFromMobile($id_cart)
    {
        if (!$id_cart) return false;

        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'jmango360_order` WHERE `mobile`=1 AND `id_cart`=' . (int)$id_cart;
        return DB::getInstance()->getValue($sql) ? true : false;
    }

    public function getTranslationWithParams($string, $source, array $parameters = array())
    {
        $translated = $this->getTranslation($string, $source);

        if (!empty($parameters) && $this->isSprintfString($string)) {
