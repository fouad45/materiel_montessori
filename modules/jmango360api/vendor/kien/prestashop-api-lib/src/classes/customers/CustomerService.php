...
class CustomerService extends BaseService
{
    ...
    public function doExecute()
    {
        if ($this->isGetMethod()) {
            ...
            if ($customer && $customer->id) {
                ...
                $this->errors = array_merge($this->errors, $this->validateController($customer));
    ...
    public function validateController(Customer $customer)
    {
        // Validate lastname
        $errors = array();
        if($customer->lastname && !call_user_func('Validate::isName', $customer->lastname)){
            $errors['lastname'] = $this->getTranslation('lastname is invalid.',
                                                            'customer-service');
            return $errors;
        }

        if($customer->firstname && !call_user_func('Validate::isName', $customer->firstname)){
            $errors['firstname'] = $this->getTranslation('firstname is invalid.',
                'customer-service');
            return $errors;
        }
