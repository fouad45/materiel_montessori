    ...
    public function doExecute()
    {
        if ($this->isPostMethod()) {
            ...
            if ($emailExist) {
                ...
                if ($this->isV17()) {
                    $message = sprintf($this->getTranslation('The email "%s" is already used, please choose another one or sign in', 'register-service'),
                        $email);
                    $this->response->errors[] = new JmError(500, $message);
                } else {
                ...
                /**
                 * PS-1512: Support 'allinone_rewards'
                 */
                if (Module::isEnabled('allinone_rewards')) {
                    ...
                    if ($request->sponsorship) {
                        $sponsorship = trim($request->sponsorship);
                        ...
                        if (Validate::isLoadedObject($sponsor) && RewardsSponsorshipModel::isCustomerAllowed($sponsor) && $sponsor->email != $customer_email) {
                            return $this->response->errors[] = new JmError(500, Tools::displayError('Invalid sponsorship email.'));
                        } else {
                ...
                try {
                    if (empty($this->errors)) {
                        ...
                        if (Module::isEnabled('allinone_rewards')) {
                            if ($request->sponsorship) {
                                $_POST['sponsorship'] = $request->sponsorship;
                            }
                        }
