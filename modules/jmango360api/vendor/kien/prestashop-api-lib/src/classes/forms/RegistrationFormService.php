...
//                $fields[] = $this->createField('confirmation', $this->getThemeTranslation('Confirm Password', 'Admin.Global'), true, 8);
            ...
            if ($moduleManager->isInstalled('ps_dataprivacy') && Module::getInstanceByName('ps_dataprivacy')->active) {
                if ((bool)(strip_tags(ConfigurationCore::get('CUSTPRIV_MSG_AUTH', $this->context->language->id)))) {
                    ...
                    if (!$this->context->customer->id) {
                        ...
                        $privacy_label = $this->createField('data_privacy_label', $privacy_message, true, 99);
    ...
    public function setCompanyInfoFields()
    {
        ...
        if (Module::isEnabled('allinone_rewards')) {
            $sponsorshipLabel = Translate::getModuleTranslation('allinone_rewards', 'Code or E-mail address of your sponsor', 'authentication');
            $fields[] = $this->createField('sponsorship', $sponsorshipLabel, false, 99);
        }
    ...
    public function convertTitle($titles)
    {
        ...
        $field['key'] = "id_gender";
        if ($this->isV17()) {
            $field['label'] = $this->getThemeTranslation('Social title', 'Shop.Forms.Labels');
        } else {
