    ...
    protected function processChangeProductInCart($update_item = true)
    {
        ...
        if ($this->id_product_attribute) {
            if (!Product::isAvailableWhenOutOfStock($product->out_of_stock)
                ...
                if ($this->isV17()) {
                    $message = $this->getTranslationWithParams('The item %product% in your cart is no longer available in this quantity. You cannot proceed with your order until the quantity is adjusted.',
                        'shopping-cart-service', array('%product%' => $product->name));
                    $this->errors[] = new JmError(500, $message);
                } else {
                    $this->errors[] = new JmError(
                        500,
...
//                        array_push($warningMessage, $error);
    ...
    protected function validateProductsAvailable($cart)
    {
        ...
        if ($this->isV17()) {
            if ($product['active']) {
                return $this->getTranslationWithParams('The item %product% in your cart is no longer available in this quantity. You cannot proceed with your order until the quantity is adjusted.',
                    'shopping-cart-service', array('%product%' => $product->name));
            }
    ...
    protected function getPayments($cart)
    {
        ...
        foreach ($results as $item) {
            ...
            $payment->logo = !empty($item['logo']) ? CommonUtils::cleanUrl($item['logo']) : '';
