    ...
    protected function transformProductList($isManufacturerCatalog = false)
    {
        ...
        foreach ($this->cat_products as $product) {
            ...
            if (strpos($_SERVER['HTTP_HOST'], 'lacoquefrancaise.com') !== false) {
                $temp = new Product($prod->id_product);
                if(isset($temp->on_sale) && $temp->on_sale == 1 && isset($temp->recommended_retail_price) && $temp->recommended_retail_price > 0 && $temp->recommended_retail_price > $prod->price) {
                    $prod->price_without_reduction = $temp->recommended_retail_price;
                }
            }
            $data[] = $prod;
        }
