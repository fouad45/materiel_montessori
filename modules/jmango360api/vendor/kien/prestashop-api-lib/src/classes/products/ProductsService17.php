...
class ProductsService17 extends ProductsService
{
    ...
    public function doExecute()
    {
        ...
        $this->jmProductSearchProvider = new \JmProductSearchProvider($this->module_name);
    ...
    protected function getProductList17()
    {
        ...
        if ($query->getIdCategory() || $query->id_products) {
            $provider = new \JmProductSearchProvider($this->module_name);
        }
    ...
    protected function setBanner(&$products)
    {
        foreach ($products as $prod) {
            ...
            if ($prod->on_sale) {
                ...
                $prod->banner_info[] = $temp;
            } elseif ($show_price && $prod->reduction && !$prod->on_sale) {
                $temp['code'] = 'on_sale';
                $temp['value'] = '1';

                if (strpos($_SERVER['HTTP_HOST'], 'packservices.it') !== false) {
                    $absoluteReduction = new \PrestaShop\Decimal\Number($prod->specific_prices->reduction);
                    $absoluteReduction = $absoluteReduction->times(new \PrestaShop\Decimal\Number('100'));
                    $negativeReduction = $absoluteReduction->toNegative();
                    $temp['label'] = sprintf('%s %s%%',
                        $this->context->getTranslator()->trans('Discount', array(), 'Shop.Theme.Catalog'),
                        $negativeReduction->round(2, \PrestaShop\Decimal\Operation\Rounding::ROUND_HALF_UP)
                    );
                } else {
                    $temp['label'] = $this->context->getTranslator()->trans('On sale!', array(), 'Shop.Theme.Catalog');
                }
    ...
    private function getProductSearchProviderFromModules($query)
    {
        ...
        if ($val->getIdCategory()) {
            $providers = new \JmProductSearchProvider($this->module_name);
        }
