...
class ProductsService16 extends ProductsService
{
    ...
    public function doExecute()
    {
        ...
        //PS-1418: return category with empty product in case category is deleted
...
//                $value['name'] = str_replace(' ', '_', $value['name']);
                ...
                if (is_array($value)) {
                    $item->name = array_key_exists('name', $value) ? $this->getTranslation($value['name'],'jm-filter') : '';
                            ...
                            if ($data) {
                                foreach (Tools::unSerialize($data) as $key_params => $params) {
                                    ...
                                    foreach ($params as $key_param => $param) {
                                        if (!isset($selected_filters[$key_params][$key_param]) && $key_params !== 'quantity') {
                                            $selected_filters[$key_params][$key_param] = array();
                                        }
                                        if ($key_params === 'quantity') {
                                            $selected_filters[$key_params][] = $this->filterVar($param);
                                        } else {
            ...
            switch ($filter['type']) {
                ...
                case 'condition':
                    $condition_array = array(
                        'new' => array('name' => Translate::getModuleTranslation('blocklayered', 'New', 'blocklayered'), 'nbr' => 0),
                        'used' => array('name' => Translate::getModuleTranslation('blocklayered', 'Used', 'blocklayered'), 'nbr' => 0),
                        'refurbished' => array('name' => Translate::getModuleTranslation('blocklayered', 'Refurbished', 'blocklayered'),
                            'nbr' => 0)
                    );
                    if (isset($products) && $products) {
                        foreach ($products as $product) {
                ...
                case 'quantity':
                    $quantity_array = array(
                        0 => array('name' => Translate::getModuleTranslation('blocklayered', 'Not available', 'blocklayered'), 'nbr' => 0),
                        1 => array('name' => Translate::getModuleTranslation('blocklayered', 'In stock', 'blocklayered'), 'nbr' => 0)
                    );
                    ...
                    $filter_blocks[] = array(
                        ...
                        'type' => 'quantity',
                        'id_key' => 0,
                        'name' => Translate::getModuleTranslation('blocklayered', 'Availability', 'blocklayered'),
                        'nameKey' => '',
                ...
                case 'manufacturer':
                    if (isset($products) && $products) {
                        ...
                        $filter_blocks[] = array(
                            ...
                            'type' => 'manufacturer',
                            'id_key' => 0,
                            'name' => Translate::getModuleTranslation('blocklayered', 'Manufacturer', 'blocklayered'),
