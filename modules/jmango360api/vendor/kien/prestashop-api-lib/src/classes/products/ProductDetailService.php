...
class ProductDetailService extends BaseService
{
    ...
    public function doExecute()
    {
         ...
         * PS-1263: Need assign above ProductDataTransform::productDetails()
         */
        $jm_product_detail->id_product = $this->product->id;
        ...
        $jm_product_detail = ProductDataTransform::productDetails($this->product, $jm_product_detail);
        ...
        $feature = $this->getProductFeatures($this->product, $jm_product_detail, $this->id_lang);
        ...
        if (strpos($_SERVER['HTTP_HOST'], 'lacoquefrancaise.com') !== false) {
            if (isset($this->product->on_sale)
                && $this->product->on_sale == 1
                && isset($this->product->recommended_retail_price)
                && $this->product->recommended_retail_price > 0
                && $this->product->recommended_retail_price > $jm_product_detail->price
            ) {
                $jm_product_detail->base_price = $this->product->recommended_retail_price;
                $jm_product_detail->calculated_base_price = $this->product->recommended_retail_price;
    ...
    protected function getProductFeatures($product, $productData, $id_lang)
    {
        $features = array();

        if (method_exists('Product', 'leoCustom')) {
            $id_product_attribute = null;

            if (empty($productData->combinations)) {
                $id_product_attribute = $product->id;
            }

            if ($id_product_attribute) {
                $leo_features = Product::leoCustom($id_product_attribute);
                if (is_array($leo_features)) {
                    foreach ($leo_features as $feature) {
                        if (!empty($feature['name'])) {
                            $features[] = array(
                                'name' => str_replace('merchant-', '', $feature['name']),
                                'value' => $feature['value']
                            );
                        }
                    }
    ...
    protected function assignAttributesGroups(JmProductDetail &$jm_product_detail)
    {
        ...
        $attributes_groups = $this->getAttributesGroups($jm_product_detail->id_product, $this->id_lang);
     ...
     * @throws
     */
    protected function getAttributesGroups($id_product, $id_lang)
    {
...
//            }
    ...
    public function assignVerifiedReview()
    {
        ...
        $productReviewService = new \ProductReviewService($this->module_name);
