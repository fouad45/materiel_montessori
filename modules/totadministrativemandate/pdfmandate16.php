...
class PDFMandate extends TCPDF
{
    /** @var Order */
    ...
    protected static $iso; // variable indicating whether a new group was requested
        /** @var array Special PDF params such encoding and font */
    protected static $pdfparams = array(); // variable containing the number of pages of the groups
    ...
    public function __construct($orientation = 'P', $unit = 'mm', $format = 'A4')
    {
        ...
        parent::__construct($orientation, $unit, $format, (strpos(Tools::strtoupper($encoding), 'UTF') !== false), $encoding);
    }
    ...
    public function header()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.5', '>')) {
            ...
            $this->Image($logo, 10, 8, $logo_width, 0, $logo_info['extension']);
        } else {
            if (file_exists(_PS_IMG_DIR_.'/logo_invoice.jpg')) {
                if ($this->isPngFile(_PS_IMG_DIR_.'/logo_invoice.jpg')) {
                    $this->Image(_PS_IMG_DIR_.'/logo_invoice.jpg', 10, 8, $logo_width, 0, 'PNG');
                } else {
                    $this->Image(_PS_IMG_DIR_.'/logo_invoice.jpg', 10, 8, $logo_width, 0);
                }
            } elseif (file_exists(_PS_IMG_DIR_.'/logo.jpg')) {
                if ($this->isPngFile(_PS_IMG_DIR_.'/logo.jpg')) {
                    $this->Image(_PS_IMG_DIR_.'/logo.jpg', 10, 8, $logo_width, 0, 'PNG');
                } else {
                    $this->Image(_PS_IMG_DIR_.'/logo.jpg', 10, 8, $logo_width, 0);
                }
            }
        }
        $this->SetFont(self::fontname(), 'B', 15);
        $this->Cell(65);
        $this->setCellMargins(0, 5);
        // Title
        $this->SetFillColor(240, 240, 240);
        $before = $this->GetY();
        $this->MultiCell(60, 10, $this->module->l('Order slip', 'pdfmandate16'), 0, 'C', true);
        // Date
        ...
        $this->MultiCell(
            190,
            5,
            $this->displayMessageUTF8($this->module->l('(only for metropolitan France and DOM / TOM)', 'pdfmandate16')),
            0,
            'C',
            false
        );
    ...
    /**
     * Set a margin for each new pages
     * @return bool
     */
    public function AcceptPageBreak()  //the function name cannot be changed
    {
    ...
    public function mandatePDF()
    {
        ...
        if ((self::$priceDisplayMethod = self::$order->getTaxCalculationMethod()) === false) {
            die($this->displayMessageUTF8($this->module->l('No price display method defined for the customer group', 'pdfmandate16')));
        }
        ...
        $this->Cell(195, 5, '> '.$this->displayMessageUTF8($this->module->l('Complete and send to:', 'pdfmandate16')), 0);
        ...
        if ($phone != false) {
            $tmp2 = $this->module->l('Tel:', 'pdfmandate16').' '.$phone;
        }
        $fax = Configuration::get('TOTADMINISTRATIVEMANDATE_FAX');
        if ($fax != false) {
            $tmp2 .= (($tmp2 != null) ? ' / ' : '').$this->module->l('Fax:', 'pdfmandate16').' '.$fax;
        }
        $mail = Configuration::get('TOTADMINISTRATIVEMANDATE_MAIL');
        if ($mail != false) {
            $tmp2 .= (($tmp2 != null) ? ' / ' : '').$this->module->l('Mail:', 'pdfmandate16').' '.$mail;
        }
        ...
        $this->Cell(
            0,
            5,
            $this->displayMessageUTF8($this->module->l('Stamp of the institution + date:', 'pdfmandate16')),
            'LTR'
        );
        $this->Ln(5);
        $this->Cell(0, 15, '', 'LBR');
        ...
        $this->Cell(0, 5, $this->displayMessageUTF8($this->module->l('Name and title of authorized person:', 'pdfmandate16')), 'LTR');
        $this->Ln(5);

        $email_text = '..........................................................................................';
        $email_text .= '@..........................................................................................';

        $this->Cell(0, 5, $this->displayMessageUTF8($this->module->l('E-mail address:', 'pdfmandate16')).$email_text, 'LR');
        $this->Ln(5);
        $this->Cell(0, 5, $this->displayMessageUTF8($this->module->l('Tel:', 'pdfmandate16')), 'LBR');
        $this->Ln(5);
        // Second content
        ...
        if (Configuration::get('VATNUMBER_MANAGEMENT')
            && isset($addressType['invoice']['addressObject'])
        ...
        if (isset($addressType['invoice']['addressObject']) && $addressType['invoice']['addressObject']->dni != null) {
            $dni = Tools::iconv('utf-8', self::encoding(), $addressType['invoice']['addressObject']->dni);
            $this->Cell(
                $width,
                10,
                $this->displayMessageUTF8($this->module->l('Tax ID number:', 'pdfmandate16')).' '.$dni,
                0,
                'L'
            );
        }
        ...
        $this->Cell(
            $width,
            10,
            $this->displayMessageUTF8($this->module->l('Order reference:', 'pdfmandate16')).' '.$reference,
            0,
            0,
            'L'
        );
        ...
        if (!self::$delivery) {
            ...
            // $shipping_cost_tax_excl = Tools::ps_round((float) self::$order->total_shipping_tax_excl, 4);

            // $gift_cost_tax_excl = Tools::ps_round((float) self::$order->total_wrapping_tax_excl, 4);

            $total_discounts = self::$order->total_discounts_tax_incl;
            ...
            if (Configuration::get('PS_TAX') || $order->total_products_wt != $order->total_products) {
                ...
                $tax_excl = $this->displayMessageUTF8($this->module->l('Total products (tax excl.)', 'pdfmandate16'));
                $tax_incl = $this->displayMessageUTF8($this->module->l('Total products (tax incl.)', 'pdfmandate16'));
                ...
                $this->Cell(
                    $width,
                    0,
                    $this->displayMessageUTF8($this->module->l('Total products ', 'pdfmandate16')).' : ',
                    0,
                    0,
                    'R'
                );
            ...
            if (!self::$orderSlip && $total_discounts != '0.00') {
                $this->Cell(
                    $width,
                    0,
                    $this->displayMessageUTF8($this->module->l('Total discounts (tax incl.)', 'pdfmandate16')).' : ',
                    0,
                    0,
                    'R'
                );
            ...
            if (isset(self::$order->total_wrapping) && ((float) (self::$order->total_wrapping) > 0)) {
                $this->Cell(
                    $width,
                    0,
                    $this->displayMessageUTF8($this->module->l('Total gift-wrapping', 'pdfmandate16')).' : ',
                    0,
                    0,
                    'R'
                );
            ...
            if (self::$order->total_shipping != '0.00' &&
                ...
                if (self::$priceDisplayMethod == PS_TAX_EXC) {
                    $this->Cell(
                        $width,
                        0,
                        $this->displayMessageUTF8($this->module->l('Total shipping (tax excl.)', 'pdfmandate16')).' : ',
                        0,
                        0,
                        'R'
                    );
                    $amount = Tools::ps_round(self::$order->total_shipping_tax_excl, 4);
                    $this->Cell(
                        0,
                        0,
                        $order_slip_prefix[0].self::convertSign(Tools::displayPrice($amount, self::$currency, true)),
                    ...
                    $this->Cell(
                        $width,
                        0,
                        $this->displayMessageUTF8($this->module->l('Total shipping (tax incl.)', 'pdfmandate16')).' : ',
                        0,
                        0,
                        'R'
                    );
            ...
            if (Configuration::get('PS_TAX') or $order->total_products_wt != $order->total_products) {
                $tax_incl = $this->displayMessageUTF8($this->module->l(' (tax incl.)', 'pdfmandate16'));
                $tax_excl = $this->displayMessageUTF8($this->module->l(' (tax excl.)', 'pdfmandate16'));
                ...
                $this->Cell(
                    $width,
                    0,
                    $this->displayMessageUTF8($this->module->l('Total', 'pdfmandate16')).' '.$tax_display.' : ',
                    0,
                    0,
                    'R'
                );
                ...
                $this->Cell(
                    $width,
                    0,
                    $this->displayMessageUTF8($this->module->l('Total', 'pdfmandate16')).' '.$tax_incl.' : ',
                    0,
                    0,
                    'R'
                );
                ...
                $this->Cell(
                    $width,
                    0,
                    $this->displayMessageUTF8($this->module->l('Total', 'pdfmandate16')).' : ',
                    0,
                    0,
                    'R'
                );
        ...
        $this->Ln(12);
        if ($this->getStringHeight(0, $text) > ($this->getPageHeight() - $this->getFooterHeight() - $this->GetY())) {
            $this->AddPage();
            $this->Ln();
            $this->SetX(10);
            $this->SetY(30);
        }
        $this->Cell(0, 6, $this->displayMessageUTF8($this->module->l('RIB', 'pdfmandate16')), 1, 2, 'C', 1);

        $this->MultiCell(0, 6, html_entity_decode($text), 1, 'L', 0);

        $this->Ln(5);

        if (version_compare(_PS_VERSION_, '1.5', '>')) {
            Hook::exec('PDFInvoice', array('pdf' => $this, 'id_order' => $id_order, 'object' => self::$order));
        } else {
    ...
    public function generateHeaderAddresses($order, $addressType, $patternRules, $width)
    {
        ...
        foreach (array_keys($addressType) as $type) {
            if ($type == 'invoice') {
                $message = $this->module->l('Address invoice', 'pdfmandate16');
                $border = 'RLT';
            } else {
                $message = $this->module->l('Address delivery', 'pdfmandate16');
                $border = 'LT';
            }
    ...
    public function prodTab($delivery = false)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!isset($this->prodTaxDetails[ (string) $product['tax_rate'] ])) {
            ...
            $this->prodTaxDetails[ (string) $product['tax_rate'] ]['total_tax_excl'] += $product['total_price_tax_excl'];
            $this->prodTaxDetails[ (string) $product['tax_rate'] ]['total_tax'] += $product['total_price_tax_excl'] * ($product['tax_rate'] / 100);
            $this->prodTaxDetails[ (string) $product['tax_rate'] ]['ecotax'] += $product['ecotax'] * $product['product_quantity'];
            $this->prodTaxDetails[ (string) $product['tax_rate'] ]['total_tax_incl'] += $product['total_price_tax_incl'];
            ...
            if (!$delivery || ($quantity > 0)) {
                ...
                if (isset($customizedDatas[$id_product][$ipa])) {
                    ...
                    foreach ($customizedDatas[$id_product][$ipa] as $customizedData) {
                        ...
                        if ($nb_images > 0) {
                            ...
                            $custoLabelTmp .= $this->displayMessageUTF8($this->module->l('image(s)', 'pdfmandate16'))."\n";
                        }
                    ...
                    $full_name .= ' - '.$this->displayMessageUTF8($this->module->l('Customized', 'pdfmandate16'))." \n";
                    ...
                    $this->Cell($w[++$i], $lineSize, $product['product_reference'], 'BLR', 1, 'L', 0, '', 4);
                ...
                if ($quantity) {
                    ...
                    $this->MultiCell($w[++$i], 5, $product_name, 'LB', 'L', 0, 1, '', '', 1, 2);
                    ...
                    $size = Tools::strlen($product_reference);
                    $product_reference = Tools::substr($product_reference, 0, 7);
                    if (Tools::strlen($product_reference) != $size) {
                        $product_reference .= '...';
                    }
                    $this->Cell($w[++$i], $lineSize, $product_reference, 'B', 0, 'L', 0, '', 0);
    ...
    public function prodTabHeader($delivery = false)
    {
        if (!$delivery) {
            $header = array(
                array($this->displayMessageUTF8($this->module->l('Description', 'pdfmandate16')), 'L'),
                array($this->displayMessageUTF8($this->module->l('Reference', 'pdfmandate16')), 'L'),
                array($this->displayMessageUTF8($this->module->l('U. price', 'pdfmandate16')), 'R'),
                array($this->displayMessageUTF8($this->module->l('Qty', 'pdfmandate16')), 'C'),
                array($this->displayMessageUTF8($this->module->l('Total', 'pdfmandate16')), 'R'),
            );
            $w = array(100, 15, 30, 15, 30);
        } else {
            $header = array(
    ...
    /**
     * Converts a price to the PDF encoding; call after Tools::displayPrice.
     *
     * @param string $s the price to encode
     *
     * @return string the price with encoded symbol
     */
    protected static function convertSign($s)
    {
        //Check if string already has a replacement (PS1.6)
        foreach (array(chr(128), chr(163), chr(165)) as $currency) {
            if (strpos($s, $currency) !== false) {
                //If symbol was already replaced, convert from iso-8859-1
                return mb_convert_encoding($s, self::encoding(), 'iso-8859-1');
            }
        }

        //Else convert from utf-8
        return mb_convert_encoding($s, self::encoding(), 'utf-8');
    }
    ...
    public function discTab()
    {
        ...
        foreach ($discounts as $discount) {
            $this->Cell(
                $w[0],
                6,
                $this->displayMessageUTF8($this->module->l('Discount:', 'pdfmandate16')).' '.$discount['name'],
                'B'
            );
            $this->Cell(
                $w[1],
    ...
    public function taxTab()
    {
        ...
        if ($vat_number_management
            ...
            $this->Ln();
            $msg = $this->module->l('Exempt of VAT according section 259B of the General Tax Code.', 'pdfmandate16');
            $this->Cell(
                30,
                0,
        ...
        if ($this->hasEcotax) {
            $header = array(
                $this->displayMessageUTF8($this->module->l('Tax detail', 'pdfmandate16')),
                $this->displayMessageUTF8($this->module->l('Tax', 'pdfmandate16')),
                $this->displayMessageUTF8($this->module->l('Pre-Tax Total', 'pdfmandate16')),
                $this->displayMessageUTF8($this->module->l('Total Tax', 'pdfmandate16')),
                $this->displayMessageUTF8($this->module->l('Ecotax (Tax Incl.)', 'pdfmandate16')),
                $this->displayMessageUTF8($this->module->l('Total with Tax', 'pdfmandate16')),
            );
        ...
        foreach ($this->prodTaxDetails as $rate => $data) {
            if ($rate != '0.00') {
                ...
                $this->Cell(
                    $w[0],
                    $lineSize,
                    $this->displayMessageUTF8($this->module->l('Products', 'pdfmandate16')),
                    0,
                    0,
                    'R'
                );
...
/*
        ...
        if ( self::$order->total_discounts != '0.00' ){
            ...
            $discount_tax_excl = self::$order->total_discounts_tax_excl;
            $discount_tax_incl = self::$order->total_discounts_tax_incl;

            $discount_tax_amount = $discount_tax_incl - $discount_tax_excl;

            $discount_tax_rate = $discount_tax_amount * 100 / $discount_tax_excl;

            $this->SetXY($this->GetX(), $this->GetY() - $lineSize + 3);
            $this->Cell(
                $w[0],
                $lineSize,
                $this->displayMessageUTF8($this->module->l('Reductions', 'pdfmandate16')),
                0,
                0,
                'R'
            );
            $this->Cell(
                $w[1],
                $lineSize,
                number_format($discount_tax_rate, 3, ',', ' ') . ' %',
                0,
                0,
                'R'
            );
            $this->Cell(
                $w[2],
                $lineSize,
                $order_slip . self::convertSign(Tools::displayPrice($discount_tax_excl, self::$currency, true)),
                0,
                0,
                'R'
            );
            $this->Cell(
                $w[3],
                $lineSize,
                $order_slip . self::convertSign(Tools::displayPrice($discount_tax_amount, self::$currency, true)),
                0,
                0,
                'R'
            );
            ...
            $this->Cell(
                //$w[$priceBreakDown['hasEcotax'] ? 5 : 4],
                $w[$this->hasEcotax ? 5 : 4],
                $lineSize,
                $order_slip . self::convertSign(Tools::displayPrice($discount_tax_incl, self::$currency, true)),
                0,
                0,
                'R'
            );
...
*/
        ...
        if ($carrier_tax_rate
            ...
            $this->Cell(
                $w[0],
                $lineSize,
                $this->displayMessageUTF8($this->module->l('Carrier', 'pdfmandate16')),
                0,
                0,
                'R'
            );
        ...
        if (self::$order->total_wrapping && self::$order->total_wrapping != '0.00') {
            ...
            $this->Cell(
                $w[0],
                $lineSize,
                $this->displayMessageUTF8($this->module->l('Gift-wrapping', 'pdfmandate16')),
                0,
                0,
                'R'
            );
        ...
        if (!$nb_tax) {
            $this->Cell(
                190,
                10,
                $this->displayMessageUTF8($this->module->l('No tax', 'pdfmandate16')),
                0,
                0,
                'C'
            );
    ...
    public function getFooterHeight($conf = array())
    {
        if (empty($conf)) {
            $conf = $this->getDetailFooter();
        }
        ...
        return (21.0 + (4 * $totalLineDetailFooter)) + ($y_delta * 7.0);
    }
    ...
    public function footer()
    {
        $conf = $this->getDetailFooter();
        $merchantDetailFooter = $this->builMerchantFooterDetail($conf);
        $heightFooter = $this->getFooterHeight($conf);
        $this->SetY(-$heightFooter);
        $this->SetFont(self::fontname(), '', 7);
        $this->Cell(
            190,
            5,
            ' '."\n".Tools::iconv('utf-8', self::encoding(), 'P. '). $this->getAliasNumPage() . '/' . $this->getAliasNbPages(),
            'T',
            1,
            'R'
        );
    ...
    private function builMerchantFooterDetail($conf)
    {
        ...
        if (Country::getIsoById(Configuration::get('PS_COUNTRY_DEFAULT')) == 'US') {
            ...
            $footerText = $this->displayMessageUTF8($this->module->l('Headquarters:', 'pdfmandate16'))."\n";
            ...
            if (isset($conf['PS_SHOP_PHONE']) && !empty($conf['PS_SHOP_PHONE'])) {
                $footerText .= $this->displayMessageUTF8($this->module->l('PHONE:', 'pdfmandate16')).' '.$conf['PS_SHOP_PHONE']."\n";
            }
        } else {
            $footerText .= $conf['PS_SHOP_NAME_UPPER'].' ';
            ...
            if (isset($conf['PS_SHOP_DETAILS']) && !empty($conf['PS_SHOP_DETAILS'])) {
                $footerText .= $this->displayMessageUTF8($this->module->l('Details:', 'pdfmandate16')).' ';
