...
{else}
	<style type="text/css">
		input.exclusive_large.hideOnSubmit {
			height: 30px;
		}
	</style>
{/if}

<h2>{l s='Order summary' mod='totadministrativemandate'}</h2>
...
{else}
	...
	<form action="{$this_path_ssl|escape:'htmlall':'UTF-8'}validation.php" method="post">
		<p>
			<img src="{$this_path|escape:'htmlall':'UTF-8'}/views/img/logo-payment.png" alt="{l s='Administrative mandate payment' mod='totadministrativemandate'}" width="86" height="49" style="float:left; margin: 0px 10px 5px 0px;" />
		...
		<p class="cart_navigation">
			...
			<input type="submit" name="submit" value="{l s='I confirm my order' mod='totadministrativemandate'}" class="exclusive_large hideOnSubmit" />
		</p>
	</form>
