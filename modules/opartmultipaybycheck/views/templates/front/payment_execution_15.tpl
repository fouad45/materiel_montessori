...
<form action="{$link->getModuleLink('opartmultipaybycheck', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
	<div class="box ompbc_box">
		...
		<p class="cart_navigation" id="cart_navigation">
			<input type="submit" value="{l s='I confirm my order' mod='opartmultipaybycheck'}" class="exclusive_large"/>
			<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" class="button_large">{l s='Other payment methods' mod='opartmultipaybycheck'}</a>
		</p>
	</div>
</form>
{/if}
